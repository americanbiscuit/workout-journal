...
export async function onRequestGet(context) {
  ...
  const rows = await context.env.DB.prepare(
    "SELECT id, name, sections, is_default, created_at, updated_at FROM day_templates WHERE user_id = ? ORDER BY is_default DESC, name ASC"
  ).bind(user.id).all();
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const result = await context.env.DB.prepare(
      "INSERT INTO day_templates (user_id, name, sections, is_default, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?) RETURNING id"
    ).bind(user.id, name, JSON.stringify(sections), isDefault, now, now).first();
